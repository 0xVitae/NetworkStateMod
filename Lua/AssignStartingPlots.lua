...
function AssignStartingPlots:BalanceAndAssign()
	...
	local avoid_lists = {};
	-- NETWORK STATE MOD: region priorities of coastal-bias civs, kept separate from
	-- priority_lists so these civs are not double-processed by the Region Priority handler.
	local coastal_priority_lists = {};
	...
	for loop = 1, self.iNumCivs do
		...
		if bNeedsCoastalStart == true then
			...
			-- NETWORK STATE MOD: a coastal-bias civ may also carry a region priority
			-- (e.g. coastal jungle); record it so coastal placement can honor it.
			if GetNumStartRegionPriorityForCiv(civType) > 0 then
				coastal_priority_lists[playerNum] = GetStartRegionPriorityListForCiv_GetIDs(civType);
			end
	...
	if iNumCoastalCivs > 0 then
		...
		if iNumCoastalCivs - iNumUnassignableCoastStarts > 0 then
			...
			-- NETWORK STATE MOD: among these, civs that also have a region priority
			-- choose ahead of generic coastal civs, so a matching coastal region
			-- (e.g. coastal jungle) is still available when they pick.
			for loop, iPlayerNum in ipairs(shuffled_non_priority_coastal_start) do
				if coastal_priority_lists[iPlayerNum] ~= nil then
					table.insert(shuffled_coastal_civs, iPlayerNum);
				end
			end
			for loop, iPlayerNum in ipairs(shuffled_non_priority_coastal_start) do
			...
			for loop, playerNum in ipairs(shuffled_coastal_civs) do
				...
				if loop <= iNumRegionsWithCoastalStart then
					-- NETWORK STATE MOD: if this civ also has a region priority, look
					-- through the still-unassigned coastal regions for one of a matching
					-- type and swap it into this civ's slot. Falls back to the default
					-- random coastal region when no match exists.
					if coastal_priority_lists[playerNum] ~= nil then
						for test_index = loop, iNumRegionsWithCoastalStart do
							local test_region = shuffled_coastal_regions[test_index];
							local bTypeMatch = false;
							for inner_loop, iPriorityType in ipairs(coastal_priority_lists[playerNum]) do
								if self.regionTypes[test_region] == iPriorityType then
									bTypeMatch = true;
									break
								end
							end
							if bTypeMatch then
								print("Coastal Bias civ", playerNum, "matched to priority Region#", test_region, "of type", self.regionTypes[test_region]);
								shuffled_coastal_regions[test_index] = shuffled_coastal_regions[loop];
								shuffled_coastal_regions[loop] = test_region;
								break
							end
						end
					end
