...
function TipHandler( control )
	...
	elseif (action.Type == "MISSION_TRADE") then
		...
		if (not bDisabled) then
			-- Network State mod: the Startup Founder's Trade Mission grants
			-- 200 total Influence (engine shows the standard amount; the rest
			-- is added by NetworkState.lua) and charters the City-State as a
			-- Special Economic Zone.
			local iInfluence = unit:GetTradeInfluence(unit:GetPlot());
			local pFounderInfo = GameInfo.Units["UNIT_NETWORK_MERCHANT"];
			local bIsFounder = (pFounderInfo ~= nil and unit:GetUnitType() == pFounderInfo.ID);
			if (bIsFounder) then
				local iBaseInfluence = GameDefines.MINOR_FRIENDSHIP_FROM_TRADE_MISSION or 30;
				iInfluence = iInfluence + (200 - iBaseInfluence);
			end
			strToolTip = strToolTip .. "[NEWLINE]----------------[NEWLINE]";
			strToolTip = strToolTip .. "+" .. iInfluence .. " [ICON_INFLUENCE]";
			strToolTip = strToolTip .. "[NEWLINE]";
			strToolTip = strToolTip .. "+" .. unit:GetTradeGold(unit:GetPlot()) .. "[ICON_GOLD]";
			if (bIsFounder) then
				strToolTip = strToolTip .. "[NEWLINE]";
				strToolTip = strToolTip .. "Becomes a Special Economic Zone: provides 25% of its [ICON_GOLD] Gold each turn";
			end
		end
	...
	-- Title
    local text = action.TextKey or action.Type or "Action"..(buttonIndex - 1);
    local strTitle = Locale.ConvertTextKey( text );
    -- Network State mod: the Startup Founder's Trade Mission is presented
    -- as converting the City-State into a Special Economic Zone.
    if (action.Type == "MISSION_TRADE" and unit ~= nil) then
        local pFounderInfo = GameInfo.Units["UNIT_NETWORK_MERCHANT"];
        if (pFounderInfo ~= nil and unit:GetUnitType() == pFounderInfo.ID) then
            strTitle = "Convert to Special Economic Zone";
        end
    end
    local strTitleString = "[COLOR_POSITIVE_TEXT]" .. strTitle .. "[ENDCOLOR]".. strBuildTurnsString;
...
print("[NetworkState] UnitPanel (SEZ-aware) loaded from mod")
