...
-- =============================================================================
-- Network State mod: Special Economic Zone income.
-- NetworkState.lua grants 25% of each SEZ City-State's gross Gold per turn
-- outside the engine's yield system; this file mirrors that math so the top
-- bar gold number and tooltip show the player's true per-turn totals.
-- (Science from City-States flows through the engine's ally-science policy
-- mechanism, so the engine displays it natively — no patching needed.)
-- SEZ status is read from the savegame key "NetworkState_SEZ_<playerID>".
-- =============================================================================
local g_NetworkStateSEZCache = nil;
local function NetworkState_GetSEZGold()
	if (g_NetworkStateSEZCache == nil) then
		g_NetworkStateSEZCache = {};
		local saveData = Modding.OpenSaveData();
		local s = saveData.GetValue("NetworkState_SEZ_" .. Game.GetActivePlayer());
		if (type(s) == "string") then
			for id in string.gmatch(s, "%d+") do
				g_NetworkStateSEZCache[tonumber(id)] = true;
			end
		end
	end
	local iGold = 0;
	for minorID in pairs(g_NetworkStateSEZCache) do
		local pMinor = Players[minorID];
		if (pMinor ~= nil and pMinor:IsAlive() and pMinor:IsMinorCiv()) then
			iGold = iGold + math.floor(pMinor:CalculateGrossGold() * 25 / 100);
		end
	end
	return iGold;
end
LuaEvents.NetworkStateSEZChanged.Add(function() g_NetworkStateSEZCache = nil; end);
Events.GameplaySetActivePlayer.Add(function() g_NetworkStateSEZCache = nil; end);
...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			local iGoldPerTurn = pPlayer:CalculateGoldRate();
			-- Network State mod: include SEZ gold in the headline number
			iGoldPerTurn = iGoldPerTurn + NetworkState_GetSEZGold();
...
function GoldTipHandler( control )
	...
	-- Network State mod: gold from Special Economic Zones
	local iSEZGoldTip = NetworkState_GetSEZGold();
	local fTotalIncome = fGoldPerTurnFromCities + iGoldPerTurnFromOtherPlayers + fCityConnectionGold + iGoldPerTurnFromReligion + fTradeRouteGold + fTraitGold + iSEZGoldTip;
	...
	if (iSEZGoldTip > 0) then
		strText = strText .. "[NEWLINE]  [ICON_BULLET]" .. iSEZGoldTip .. " from Special Economic Zones";
	end
...
UpdateData();
DoInitTooltips();
print("[NetworkState] TopPanel (SEZ-aware) loaded from mod")
